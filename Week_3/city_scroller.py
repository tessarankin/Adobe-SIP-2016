import pygame
import random

# Define some colors
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
GREEN = (0, 255, 0)
RED = (255, 0, 0)
BLUE = (0, 0, 255)
GREY = (129, 129, 129)
colors = [BLACK, WHITE, GREEN, BLUE, RED]
 
def random_color():
  return random.choice(colors)

pygame.init()
 
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
 
class Building():
  def __init__(self, x_point, y_point, width, height, color):
    self.x_point= x_point
    self.y_point= y_point
    self.width= width
    self.height= height
    self.color= color

  def draw(self):
    pygame.draw.rect(screen, self.color, (self.x_point, SCREEN_HEIGHT - self.height, self.width, self.height))

  def move(self, speed):
    self.x_point += speed

class Scroller(object):
  
    def __init__(self, width, height, base, color, speed):
        self.width= width = random.randint(50, 100)
        self.height= height
        self.base= base
        self.color= color
        self.speed= speed
        self.building_list= []
        self.add_buildings()
        self.combined_width = 0

    def add_buildings(self):
        self.add_building(-100,100)
        random_width = 0
        x_location=0
        while x_location<= self.width:
            random_width = random.randint(50, 100)
            self.add_building(x_location, random_width)
            x_location = x_location + random_width
 
    def add_building(self, x_location, combined_width):
        building1 = Building(x_location, 10, self.width, random.randint(50, 500), self.color)
        self.building_list.append(building1)

    def draw_buildings(self):
        pygame.draw.rect(screen, self.color, (0, SCREEN_HEIGHT, self.width, SCREEN_WIDTH - self.width))
        for building1 in self.building_list:
            building1.draw()

    def move_buildings(self, runningspeed):
      for building1 in self.building_list:
        building1.move(self.speed + runningspeed)
      
        last_building = self.building_list[-1]
        if last_building.x_point > self.width:
          self.building_list.remove(last_building) 
        first_building = self.building_list[0] 
        if first_building.x_point > 0:
          building_width = random.randint(40,180)
          building1 = Building(-building_width, 0, building_width, random.randint(80, 400), random_color())
          self.building_list.insert(0,building1)

           def move_buildings(self, runningspeed):




class Moving_Object(object):
  def __init__(self, x_point, color, speed):
    self.x_point= x_point
    self.color= color
    self.runstage = 1
    self.speed = speed
    

  def draw(self):
    feet_y = SCREEN_HEIGHT 
    print(self.x_point)
    pygame.draw.circle(screen, self.color, (self.x_point + 100, feet_y + 1), 10, 0)
    

  def faster(self):
        if self.speed < 10:
            self.speed +=1

  def slower(self):
    if self.speed > 0:
        self.speed -= 1


  '''
    class Player(pygame.sprite.Sprite):
      def __init__ (self, height,  colour)
        Sprite.__init__(self)

      def run
  '''

    
FRONT_SCROLLER_COLOR = (255,255,255)
MIDDLE_SCROLLER_COLOR = BLUE
BACK_SCROLLER_COLOR = BLACK
BACKGROUND_COLOR = (106, 90, 205)
 
front_scroller = Scroller(SCREEN_WIDTH, 500, 0, FRONT_SCROLLER_COLOR, 3)
middle_scroller = Scroller(SCREEN_WIDTH, 300, 100, MIDDLE_SCROLLER_COLOR, 2)
back_scroller = Scroller(SCREEN_WIDTH, 400, 200, BACK_SCROLLER_COLOR, 1)
thingy = Moving_Object(int(SCREEN_WIDTH/2), WHITE, 0)

pygame.display.set_caption("CityScroller") 
clock = pygame.time.Clock() 
done = False
while not done:
   for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
               thingy.faster()
            elif event.key == pygame.K_RIGHT:
                thingy.slower()
        if event.type == pygame.QUIT:
            done = True
 
   screen.fill(BACKGROUND_COLOR)
 
   back_scroller.draw_buildings()
   middle_scroller.draw_buildings()
   front_scroller.draw_buildings()
   thingy.draw()

   if thingy.speed > 0:
     back_scroller.move_buildings(thingy.speed)
     middle_scroller.move_buildings(thingy.speed)
     front_scroller.move_buildings(thingy.speed)

   pygame.display.flip()
 
   clock.tick(60)

pygame.quit